...
新規カードの入力途中も下書きに含まれます。別のカードを選んで編集しても、
「下書きに戻る」で入力を再開できます。

ファイルに保存されるのは追加済みのカードです。入力途中のカードは「新規カード」で追加してください。
未追加の入力がある間は、ファイル保存後も下書きを保持し、画面上部に「未追加のカードあり」と表示します。

保存中に追加編集した場合は「未保存」のまま保持されるので、もう一度保存してください。
保存中の連打は重複実行せず、別ファイルを開く操作・新規作成は保存完了まで待ちます。
入力欄ではTab / Shift+Tabで、フォームの外も含めて順番に移動できます。
...
保存中の追加編集・重複保存・キャンセル、未追加カードの下書き復元、Tab移動も検証します。
既存のChromiumを使う場合は、`MITO_CHROMIUM_EXECUTABLE` に実行ファイルのパスを指定できます。
`tests/layout.js` も同じ実行環境で動き、パネル開閉・画面サイズ変更時のスクロール範囲、
色選択のキーボード操作、印刷時の年表表示を確認します。
